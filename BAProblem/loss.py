...
def Undistort(xp, yp, cam):
    ...
    focal = cam[:, 6]
    predicted_x = focal*xp
    predicted_y = focal*yp

    return [predicted_x, predicted_y, distortion]


def UnitSphereReprojectionError(points_ob, cameras_ob, weights, features):
    if (len(points_ob.shape) == 3):
        points_ob = points_ob[:, 0, :]
        cameras_ob = cameras_ob[:, 0, :]
    delta = 1e-6
    delta_squared = 1e-12
    undistorted_point = Undistort(features[:, 0], features[:, 1], cameras_ob)
    undistorted_point = torch.cat([undistorted_point[0].view(
        -1, 1), undistorted_point[1].view(-1, 1), undistorted_point[2].view(-1, 1)], dim=1)
    unit_point = undistorted_point/undistorted_point.norm(dim=1, keepdim=True)
    rotated_unit_point = AngleAxisRotatePoint(cameras_ob[:, :3], unit_point)

    dist = points_ob - cameras_ob[:, 3:6]
    dist_dot_rotated_unit_point = (
        dist * rotated_unit_point).sum(dim=1, keepdim=True)
    dist_squared_norm = (dist**2).sum(dim=1, keepdim=True)
    dist_squared_norm_plus_delta_squared = dist_squared_norm + delta_squared

    error = rotated_unit_point - (dist_dot_rotated_unit_point/(
        dist_squared_norm_plus_delta_squared + delta * dist_squared_norm_plus_delta_squared.sqrt())) * dist
...
def SphericalReprojectionError2D(points_ob, cameras_ob, features):
    ...
    P_ob = AngleAxisRotatePoint(
        cameras_ob[:, :3], points_ob - cameras_ob[:, 3:])
    P_ob = Normalize(P_ob)
...
def ColmapReprojectionError(points_ob, cameras_ob, features):
    ...
    r4 = r2 * r2
    r6 = r4 * r2
    coeff = 1 + cameras_ob[:, 9] * r2 + \
        cameras_ob[:, 10] * r4 + cameras_ob[:, 11] * r6
